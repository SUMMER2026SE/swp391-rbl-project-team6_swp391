  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <PageHeader
        ...
        actions={<Button asChild><Link to="/teacher/lessons/create"><Plus className="mr-2 h-4 w-4" />New lesson</Link></Button>}
      />
          ...
          return (
            <Card key={l.id} className="border-border/60">
              <CardContent className="p-4">
                <div className="mb-1 flex items-center gap-2">{cls && <LevelBadge level={cls.level} />}<StatusBadge status={l.status} /></div>
                <button onClick={() => setOpen(l.id)} className="block w-full truncate text-left font-semibold hover:text-primary">{l.title}</button>
                ...
                <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
                  <span>{cls?.name ?? "Unassigned"} · {l.skill}</span>
                  <Button size="sm" variant="ghost" onClick={() => setOpen(l.id)}><Eye className="h-4 w-4" /></Button>
                </div>
              </CardContent>
            </Card>
          );
      ...
      <PreviewSheet open={!!sel} onOpenChange={(o) => !o && setOpen(null)} title={sel?.title ?? ""} description={sel?.jpTitle}>
        {sel && (
          <div className="space-y-3 text-sm">
            <p><b>Skill:</b> {sel.skill} · <b>Duration:</b> {sel.duration} min</p>
            <p>{sel.objective}</p>
            <div className="flex gap-2">
              <Button className="flex-1" onClick={() => { toast.info("Edit"); setOpen(null); }}><Edit className="mr-2 h-4 w-4" />Edit</Button>
              <Button variant="outline" onClick={() => { toast.success("Duplicated"); setOpen(null); }}><Copy className="h-4 w-4" />Duplicate</Button>
            </div>
          </div>
        )}
      </PreviewSheet>
