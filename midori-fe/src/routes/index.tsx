...
function Landing() {
  ...
  const navButtons = !loaded ? null : user ? (
    ...
    <>
      ...
      <Link to="/register" className="px-4 py-2 rounded-xl bg-gradient-hero text-white text-sm font-semibold shadow-md hover:shadow-lg transition">Get started</Link>
    </>
  );
    ...
    <div className="min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-6 pt-10 pb-20 grid lg:grid-cols-2 gap-10 items-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <h1 className="text-5xl md:text-6xl font-extrabold font-display leading-[1.05] tracking-tight">
            Master Japanese <br/>with <span className="gradient-text">MIDORI</span>
          </h1>
          <p className="mt-5 text-lg text-muted-foreground max-w-lg">
            Grammar lessons, listening dictation, AI shadowing and real JLPT mock exams — all guided by an AI that gives instant, native-level feedback.
          </p>
          <div className="mt-7 flex flex-wrap gap-3">
            <Link to="/register" className="px-6 py-3 rounded-2xl bg-gradient-hero text-white font-semibold shadow-lg hover:shadow-xl transition flex items-center gap-2">
              Start free <ArrowRight className="w-4 h-4" />
            </Link>
            <Link to="/login" className="px-6 py-3 rounded-2xl glass font-semibold hover:bg-white/70 transition">I already have an account</Link>
          </div>
          <div className="mt-8 flex items-center gap-6 text-sm text-muted-foreground">
            <div className="flex items-center gap-1.5"><Check className="w-4 h-4 text-primary"/> No credit card</div>
            <div className="flex items-center gap-1.5"><Check className="w-4 h-4 text-primary"/> 5 JLPT levels</div>
            <div className="flex items-center gap-1.5"><Check className="w-4 h-4 text-primary"/> AI feedback</div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7 }} className="relative">
          <div className="glass rounded-3xl p-6 relative">
            <div className="flex items-center justify-between mb-4">
              <div>
                <div className="text-xs uppercase tracking-widest text-muted-foreground">Today's lesson</div>
                <div className="font-display font-bold text-xl">〜なければなりません</div>
              </div>
              <span className="px-2.5 py-1 text-[10px] rounded-full bg-jp-red/10 text-jp-red font-bold">N4</span>
            </div>
            <div className="rounded-2xl bg-gradient-sakura p-5 mb-4">
              <div className="text-3xl font-display font-bold">毎日勉強しなければなりません。</div>
              <div className="text-sm text-muted-foreground mt-1">Mainichi benkyou shinakereba narimasen.</div>
              <div className="text-sm mt-2">"I must study every day."</div>
            </div>
            <div className="grid grid-cols-3 gap-3 text-center">
              {[{l:"XP",v:"+120"},{l:"Streak",v:"32d"},{l:"Accuracy",v:"94%"}].map(s=>(
      ...
      <section id="features" className="max-w-7xl mx-auto px-6 py-16">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-extrabold font-display">Everything you need to reach <span className="gradient-text">JLPT N1</span></h2>
          <p className="mt-3 text-muted-foreground max-w-2xl mx-auto">Four AI-powered training modes built specifically for serious Japanese learners.</p>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            { icon: GraduationCap, title: "Grammar by level", desc: "Structured lessons from N5 to N1 with examples and quizzes." },
            { icon: Headphones, title: "Listening dictation", desc: "AI checks your transcription, highlights mistakes, explains why." },
            { icon: Mic, title: "AI shadowing", desc: "Pass each sentence before unlocking the next. Native-level scoring." },
            { icon: ClipboardCheck, title: "JLPT Exam Room", desc: "Full timed mock exams with AI-generated mistake explanations." },
          ].map((f, i) => (
            <motion.div key={f.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
              ...
              <div className="w-12 h-12 rounded-2xl bg-gradient-hero grid place-items-center text-white mb-4"><f.icon className="w-5 h-5"/></div>
      ...
      <section id="levels" className="max-w-7xl mx-auto px-6 py-16">
        <h2 className="text-3xl font-extrabold font-display text-center mb-10">Train at every JLPT level</h2>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {["N5","N4","N3","N2","N1"].map((l,i)=>(
            <motion.div key={l} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i*0.05 }}
