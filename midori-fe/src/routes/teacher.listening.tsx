  ...
  <div className="bg-amber-50 dark:bg-amber-950/20 rounded-2xl p-4 border border-amber-200 dark:border-amber-800/30 space-y-3">
    <div className="flex items-center gap-2 mb-1">
      ...
      <span className="text-sm font-bold text-amber-600 dark:text-amber-400">
        Blank Fill Configuration
      </span>
    </div>
    <p className="text-xs text-amber-600 dark:text-amber-300">
      Add words/phrases to hide in the exercise. These will appear as blanks for students.
    </p>
    ...
    {blankWords.length > 0 ? (
      <div className="space-y-1.5">
        <p className="text-[10px] text-amber-500 font-semibold">
          {blankWords.length} word(s) configured
        </p>
        {blankWords.map((b) => (
          <div
            key={b.id}
            className="flex items-center gap-2 bg-white dark:bg-slate-800 rounded-xl px-3 py-2"
          >
  ...
  >
    ...
    <div className="flex gap-2">
      <button
        type="button"
        onClick={onCancel}
        className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-bold hover:bg-slate-200 dark:hover:bg-slate-600 transition"
      >
        Cancel
      </button>
      <button
        ...
        className="px-4 py-2 rounded-xl bg-red-500 text-white text-xs font-bold hover:bg-red-600 transition disabled:opacity-50 flex items-center gap-1"
      >
...
function ListeningPage() {
  ...
  const handleCreateExercise = async () => {
    ...
    try {
      ...
      const msg =
        err instanceof ApiError ? err.message : "Failed to create exercise. Please try again.";
  ...
  const openEditModal = async (exercise: ListeningExercise) => {
    ...
    try {
      ...
      const exerciseTypeFromApi =
        (detail.exerciseType as ExerciseType) || parsedMeaning.type || "Dictation";
      ...
      setEditBlankWords(
        parsedMeaning.blankWords.map((w: string, idx: number) => ({ id: idx, answer: w })),
      );
      setEditPreviewText("");
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      await listeningApi.updateListening(selectedExercise.id, {
        ...
        topic:
          (editFormTopic === "__custom__" ? editFormTopicCustom.trim() : editFormTopic.trim()) ||
          "General",
      });
      await fetchAllData();
      ...
      const mappedDetail: ListeningExercise = {
        ...
        audioUrl: updatedDetail.audioUrl
          ? updatedDetail.audioUrl.startsWith("http")
            ? updatedDetail.audioUrl
            : `http://localhost:8080${updatedDetail.audioUrl}`
          : undefined,
        audioFileName: updatedDetail.audioFileName ?? undefined,
        transcript: updatedDetail.transcript || "",
        meaning: parsedMeaning.text,
        duration: "0:00",
        date: updatedDetail.createdAt
          ? new Date(updatedDetail.createdAt).toLocaleDateString()
          : "Just now",
      };
      setSelectedExercise(mappedDetail);
      setSegments(parseTranscriptToSegments(mappedDetail.transcript));
        ...
        >
          ...
          <div className="sticky top-0 bg-white dark:bg-slate-800 z-10 px-6 py-5 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between rounded-t-3xl">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Create a new listening exercise for students
              </p>
            </div>
            <button
              onClick={() => setShowNewModal(false)}
          ...
          <div className="p-6 space-y-5">
              ...
              <input
                ...
                className={`w-full px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-700 border ${
                  createErrors.title
                    ...
                    : "border-slate-200 dark:border-slate-600 focus:ring-primary/40"
            ...
            <div>
              ...
              />
              <div
                className={`border-2 border-dashed rounded-xl p-4 text-center transition cursor-pointer ${newAudio ? "border-green-400 bg-green-50/50 dark:bg-green-950/20" : "border-slate-200 dark:border-slate-600 hover:border-primary/40"}`}
                onClick={() => newFileInputRef.current?.click()}
              >
                <Upload
                  className={`w-6 h-6 mx-auto mb-1.5 ${newAudio ? "text-green-500" : "text-muted-foreground"}`}
                />
                <p
                  className={`text-sm font-semibold ${newAudio ? "text-green-600" : "text-muted-foreground"}`}
                >
            ...
            <div>
              <label className="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1.5">
                Transcript
                {newType === "Blank Fill" && (
                  <span className="text-amber-500 ml-1">(used for blank fill preview)</span>
                )}
              ...
              <textarea
                ...
                onChange={(e) => {
                  ...
                  if (createErrors.transcript)
                    setCreateErrors((prev) => ({ ...prev, transcript: undefined }));
                }}
                rows={5}
                placeholder="Paste the full transcript here...&#10;&#10;各行がセグメントになります。"
                className={`w-full px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-700 border ${
                  createErrors.transcript
                    ? "border-red-400 focus:ring-red-400/40"
                    : "border-slate-200 dark:border-slate-600 focus:ring-primary/40"
        ...
        >
          ...
          <div className="p-6 space-y-5">
            ...
            <div>
              ...
              />
              <div
                className={`border-2 border-dashed rounded-xl p-4 text-center transition cursor-pointer ${editFormAudio ? "border-green-400 bg-green-50/50 dark:bg-green-950/20" : "border-slate-200 dark:border-slate-600 hover:border-primary/40"}`}
                onClick={() => editFileInputRef.current?.click()}
              >
                <Upload
                  className={`w-6 h-6 mx-auto mb-1.5 ${editFormAudio ? "text-green-500" : "text-muted-foreground"}`}
                />
                <p
                  className={`text-sm font-semibold ${editFormAudio ? "text-green-600" : "text-muted-foreground"}`}
                >
                  {editAudioFile
                    ? `Attached: ${editAudioFile.name}`
                    : selectedExercise?.audioUrl
                      ? `Current audio attached`
                      : "Click to attach audio file"}
                </p>
                <p className="text-[10px] text-muted-foreground mt-0.5">MP3, WAV, M4A — max 20MB</p>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) => {
                  ...
                  if (editErrors.transcript)
                    setEditErrors((prev) => ({ ...prev, transcript: undefined }));
                }}
                rows={5}
                className={`w-full px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-700 border ${
                  editErrors.transcript
                    ? "border-red-400 focus:ring-red-400/40"
                    : "border-slate-200 dark:border-slate-600 focus:ring-primary/40"
                } text-sm outline-none focus:ring-2 resize-y transition`}
              />
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            {
              label: "With Audio",
              value: exercises.filter((e) => e.audio).length.toString(),
              icon: Volume2,
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          >
            {["All Types", "Dictation", "Blank Fill"].map((t) => (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <button
                        ...
                        className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 transition shadow-lg ${
                          isPlaying
                            ...
                            : "bg-slate-100 dark:bg-slate-700 text-muted-foreground hover:bg-primary/10 hover:text-primary"
                        }`}
                      >
                        {isPlaying ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${levelColors[ex.level]}`}
                          >
                            {ex.level}
                          </span>
                          <span
                            className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${typeColors[ex.type]}`}
                          >
                            {ex.type}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-3 text-[10px] text-muted-foreground flex-wrap">
                          ...
                          {ex.audio ? (
                            ...
                            <span className="flex items-center gap-1 text-slate-400">
                              <Volume2 className="w-3 h-3" /> No Audio
                            </span>
                          )}
                          {ex.transcript ? (
                            <span className="flex items-center gap-1">
                            ...
                            <span className="flex items-center gap-1 text-slate-400">
                              <FileText className="w-3 h-3" /> No Transcript
                            </span>
                          )}
                          {ex.topic ? (
                            <span className="flex items-center gap-1">
                              <Headphones className="w-3 h-3" /> {ex.topic}
                            </span>
                          ) : (
                            <span className="flex items-center gap-1 text-slate-400">
                              <List className="w-3 h-3" /> No Topic
                            </span>
                          )}
                          <span>{ex.date}</span>
                        </div>
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">
                Preview and edit your listening content
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => setIsDeleting(selectedExercise.id)}
              disabled={isDeleting !== null}
            ...
            >
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground mb-6">
                  Are you sure you want to delete "{selectedExercise.title}"? This action cannot be
                  undone.
                </p>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-slate-700">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span
              className={`px-2 py-0.5 rounded-full text-xs font-bold ${levelColors[selectedExercise.level]}`}
            >
              {selectedExercise.level}
            </span>
            <span
              className={`px-2 py-0.5 rounded-full text-xs font-bold ${typeColors[selectedExercise.type]}`}
            >
            ...
            {selectedExercise.topic ? (
              ...
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-50 dark:bg-slate-800 text-slate-400">
                No Topic
              </span>
            )}
          </div>
          ...
          {selectedExercise.type === "Blank Fill" &&
            selectedExercise.hiddenWords &&
            selectedExercise.hiddenWords.length > 0 && (
              <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-700">
                ...
                <div className="flex flex-wrap gap-1">
                  {selectedExercise.hiddenWords.map((word, idx) => (
                    <span
                      key={idx}
                      className="px-2.5 py-1 rounded-lg bg-amber-50 text-amber-600 dark:bg-amber-950/30 dark:text-amber-400 text-xs font-semibold"
                    >
                      {word}
                    </span>
                  ))}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="p-2 rounded-xl hover:bg-white/50 dark:hover:bg-white/10 transition"
                >
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`p-2 rounded-xl transition ${isLooping ? "bg-primary text-white" : "hover:bg-white/50 dark:hover:bg-white/10"}`}
                  title="Loop"
                >
              ...
              {isEditing && (
                <button
                  onClick={() =>
                    setSegments([
                      ...segments,
                      { id: Date.now(), start: "0:00", end: "0:05", text: "" },
                    ])
                  }
              ...
              {segments.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Edit this exercise to add a transcript
                  </p>
                </div>
              ) : (
                segments.map((seg, i) => {
                      ...
                      {isEditing ? (
                        <div className="flex-1 flex gap-2">
                          ...
                          <input
                            ...
                            className="flex-1 px-2 py-1 rounded bg-muted text-sm outline-none"
                        ...
                        <div className="flex-1">
                          <span
                            className={`text-sm font-semibold ${blankMode ? "text-amber-500 font-bold" : "text-slate-700 dark:text-slate-200"}`}
                          >
                            {displayText}
                          </span>
                          {isActive && (
                            <div className="w-2 h-2 rounded-full bg-primary inline-block ml-1 animate-pulse" />
                          )}
                        </div>
                      )}
                    </motion.div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  { label: "Accuracy", value: "—", color: "text-green-500" },
                  {
                    ...
                    value:
                      duration > 0 ? (
                        ...
                        <AudioDuration
                          src={selectedExercise.audioUrl}
                          fallback={selectedExercise.duration}
                        />
                      ) : (
                  ...
                  <div key={stat.label} className="text-center p-3 rounded-xl bg-muted/50">
                    <div className={`font-display font-black text-lg ${stat.color}`}>
                      {stat.value}
                    </div>
                    <div className="text-[10px] text-muted-foreground">{stat.label}</div>
                  </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
              ...
              <div className="p-4">
                <div className="p-4 rounded-xl bg-muted/30 border border-dashed border-slate-200 dark:border-slate-600 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {blankMode
                      ? "Blank fill mode: students fill in missing words"
                      : "Listen and practice mode"}
                  </p>
                </div>
