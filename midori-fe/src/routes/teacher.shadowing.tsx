...
const initialExercises: ShadowingExercise[] = [
  {
    id: 1,
    title: "Japanese Greeting — Morning",
    level: "N5",
    topic: "Daily Life",
    status: "published",
    audio: true,
    ...
    sentences: [
      {
        id: 1,
        number: 1,
        japanese: "おはようございます、田中さん。",
        romaji: "Ohayōgozaimasu, Tanaka-san.",
        meaning: "Good morning, Tanaka-san.",
        timestamp: "0:00",
      },
      {
        id: 2,
        number: 2,
        japanese: "おはよう！今日はどうですか？",
        romaji: "Ohayō! Kyō wa dō desu ka?",
      ...
      },
      {
        ...
        romaji: "Kyō wa totemo ii tenki desu ne.",
        meaning: "It's a very nice weather today.",
        timestamp: "0:22",
      },
      {
        id: 4,
        number: 4,
        japanese: "ええ、でも午後は雨が降るそうです。",
        romaji: "Ee, demo gogo wa ame ga furu sō desu.",
        meaning: "Yes, but it seems it will rain in the afternoon.",
        timestamp: "0:32",
      },
      {
        id: 5,
    ...
    sentences: [
      {
        id: 1,
        number: 1,
        japanese: "はじめまして、田中太郎と申します。",
        romaji: "Hajimemashite, Tanaka Tarō to moshimasu.",
        meaning: "Nice to meet you, my name is Tanaka Tarō.",
        timestamp: "0:00",
      },
      {
        id: 2,
        number: 2,
        japanese: "ABC株式会社の営業部に所属しております。",
        romaji: "ABC kabushikigaisha no eigyōbu ni shozo-chō itashimasu.",
        meaning: "I belong to the sales department at ABC Corporation.",
        timestamp: "0:15",
      },
      {
        id: 3,
  ...
  },
  {
    ...
    sentences: [
      ...
      },
      {
        ...
        meaning: "Have you decided what you'd like to order?",
        timestamp: "0:10",
      },
    ],
    duration: "3:00",
  ...
  },
  {
    ...
    sentences: [
      ...
      },
      {
        ...
        meaning: "Please go straight along this road.",
        timestamp: "0:10",
      },
    ],
    duration: "5:30",
    ...
    sentences: [
      {
        id: 1,
        number: 1,
        japanese: "はい、ABC株式会社です。",
        romaji: "Hai, ABC kabushikigaisha desu.",
        meaning: "Hello, ABC Corporation.",
        timestamp: "0:00",
      },
      {
        id: 2,
        ...
        romaji: "Tanaka-san o onegaishimasu.",
        meaning: "I'd like to speak with Tanaka-san.",
        timestamp: "0:08",
      },
    ],
    duration: "3:45",
    ...
    sentences: [
      {
        ...
        number: 1,
        japanese: "今日は何時間目ですか。",
        romaji: "Kyō wa nan-jikanme desu ka?",
        meaning: "What period is it today?",
        timestamp: "0:00",
      },
      {
        id: 2,
        number: 2,
        japanese: "今日は体育の時間です。",
        romaji: "Kyō wa taiiku no jikan desu.",
        meaning: "Today is PE class.",
        timestamp: "0:08",
      },
    ],
    duration: "2:00",
...
function TranscriptEditor({ sentences, onChange, compact }: TranscriptEditorProps) {
  ...
  const addSentence = () => {
    ...
    onChange([
      ...sentences,
      { id: Date.now(), number: next, japanese: "", romaji: "", meaning: "", timestamp: "0:00" },
    ]);
  };
      ...
      {sentences.map((s, i) => (
        <div key={s.id} className="bg-[var(--accent)]/40 rounded-xl p-3 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <div
              className={`w-7 h-7 rounded-full bg-gradient-hero text-white text-xs font-bold flex items-center justify-center flex-shrink-0`}
            >
              {s.number}
            </div>
            <input
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-[var(--border)] rounded-t-3xl flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {mode === "new"
                ? "Create a new shadowing exercise for students"
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div>
            ...
            >
              <Upload
                className={`w-6 h-6 mx-auto mb-1.5 ${audio ? "text-green-500" : "text-muted-foreground"}`}
              />
              <p
                className={`text-sm font-semibold ${audio ? "text-green-600" : "text-muted-foreground"}`}
              >
        ...
        <div className="flex-shrink-0 px-6 py-4 border-t border-[var(--border)] flex items-center justify-end gap-3 rounded-b-3xl bg-[var(--card)]">
          ...
          >
            ...
            {mode === "new" ? "Create Exercise" : "Save Changes"}
          </button>
        </div>
      </motion.div>
    </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span
                className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${topicColorMap[exercise.topic] ?? topicColorMap["Custom"]}`}
              >
                {exercise.topic}
              </span>
              <span
                className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${levelColors[exercise.level]}`}
              >
                {exercise.level}
              </span>
              <span
        ...
        <div className="mb-4">
          ...
          <div className="space-y-2">
            {exercise.sentences.map((s) => (
              <div key={s.id} className="flex items-center gap-3 p-3 rounded-xl bg-muted/40">
                ...
                <div className="flex-1">
                  ...
                  {s.romaji && (
                    <div className="text-[10px] text-muted-foreground italic">{s.romaji}</div>
                  )}
                  {s.meaning && (
            ...
            {exercise.sentences.length === 0 && (
              <p className="text-xs text-muted-foreground text-center py-4">
                No sentences in this exercise yet.
              </p>
            )}
          </div>
        </div>
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Upload native sentences, manage transcripts, and review AI pronunciation analysis
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          {
            label: "AI Analysis",
            value: exercises.filter((e) => e.aiAnalysis).length.toString(),
            icon: Bot,
            color: "bg-purple-50 text-purple-500",
          },
          {
            label: "Total Plays",
            value: exercises.reduce((a, e) => a + e.completions, 0).toLocaleString(),
            icon: Play,
          ...
          },
          {
            label: "Avg. Accuracy",
            value:
              exercises.filter((e) => e.accuracy > 0).length > 0
                ? `${Math.round(exercises.filter((e) => e.accuracy > 0).reduce((a, e) => a + e.accuracy, 0) / exercises.filter((e) => e.accuracy > 0).length)}%`
                : "—",
            icon: CheckCircle,
      ...
      <div className="relative">
        ...
        >
          ...
          <span
            className={
              topicFilter !== "All Topics" ? "text-primary font-bold" : "text-muted-foreground"
            }
          >
            {topicFilter}
          </span>
          ...
          <ChevronDown
            className={`w-4 h-4 text-muted-foreground ml-1 transition-transform duration-200 ${topicOpen ? "rotate-180" : ""}`}
          />
        </button>
            ...
            >
              ...
              {DROPDOWN_TOPICS.map((t) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-2 h-2 rounded-full flex-shrink-0 ${topicFilter === t.label ? "bg-white" : t.color.replace("text-", "bg-")}`}
                    />
                    {t.label}
                    {count > 0 && (
                      <span
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((ex, i) => (
            ...
            >
              {/* Top bar */}
              <div
                className={`h-2 ${ex.level === "N5" ? "bg-blue-400" : ex.level === "N4" ? "bg-green-400" : ex.level === "N3" ? "bg-yellow-400" : ex.level === "N2" ? "bg-orange-400" : "bg-red-400"}`}
              />
              ...
              <div className="p-5">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-muted-foreground mt-0.5 flex-wrap">
                        <span
                          className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${topicColorMap[ex.topic] ?? topicColorMap["Custom"]}`}
                        >
                          {ex.topic}
                        </span>
                ...
                <div className="bg-[var(--accent)]/30 rounded-xl p-3 mb-3 text-xs text-muted-foreground font-mono leading-relaxed">
                  ...
                  {ex.sentences.length > 3 && (
                    <div className="opacity-40">... and {ex.sentences.length - 3} more</div>
                  )}
                  {ex.sentences.length === 0 && (
                    <div className="opacity-40 italic">No sentences yet</div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-0.5 h-6 mb-3">
                  {Array.from({ length: 30 }).map((_, wi) => (
                    <div
                      ...
                      style={{
                        height: `${20 + Math.sin(wi * 0.8) * 40 + Math.random() * 20}%`,
                        minHeight: "2px",
                      }}
                    />
                  ))}
