    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-secondary-col mt-0.5">Manage JLPT shadowing practice lessons</p>
        </div>
              ...
              <div key={level} className="space-y-3">
                ...
                <div className="grid gap-3">
                  {itemsByLevel[level].map((item) => (
                    <ShadowingCard key={item.id} item={item} onView={() => setViewingItem(item)} onEdit={() => setEditingItem(item)} onDelete={() => handleDelete(item.id)} />
                  ))}
                </div>
              </div>
            ) : null
      ...
      <AnimatePresence>
        {viewingItem && (
          <ShadowingDetailModal item={viewingItem} onClose={() => setViewingItem(null)} onEdit={() => { setEditingItem(viewingItem); setViewingItem(null); }} />
        )}
      </AnimatePresence>
...
// ─── Shadowing Card ──────────────────────────────────────────────────────────────
function ShadowingCard({ item, onView, onEdit, onDelete }: { item: ShadowingItem; onView: () => void; onEdit: () => void; onDelete: () => void }) {
          ...
          <div className="p-4 rounded-xl bg-primary/5 border border-primary/10">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-muted-col text-xs mt-2">Audio: {item.audioUrl}</p>
              </div>
            </div>
          </div>
                    ...
                    <div className="flex-1">
                      ...
                      {activeSegment === i && (
                        <div className="mt-2 space-y-1">
                          ...
                          <p className="text-muted-col text-xs">Time: {seg.startTime}s - {seg.endTime}s</p>
                        </div>
                      )}
                    </div>
                  </div>
...
// ─── Shadowing Form Modal ────────────────────────────────────────────────────────
function ShadowingFormModal({ item, onClose, onSubmit }: { item: ShadowingItem | null; onClose: () => void; onSubmit: (item: ShadowingItem) => void }) {
  ...
  const [segments, setSegments] = useState<ShadowingSegment[]>(
    item?.script || [{ id: "seg-1", startTime: 0, endTime: 5, text: "", translation: "" }]
  );
  ...
  return (
    <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b separator">
          <h2 className="font-display font-bold text-primary-col text-lg">{item ? "Edit Lesson" : "Add Shadowing Lesson"}</h2>
          <button onClick={onClose} className="p-2 rounded-xl glass-surface text-secondary-col hover:text-primary-col">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-auto p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="N5 Basic Greetings" className="w-full px-4 py-3 rounded-xl input-glass text-sm" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={audioUrl} onChange={(e) => setAudioUrl(e.target.value)} placeholder="/audio/n5-greetings.mp3" className="w-full px-4 py-3 rounded-xl input-glass text-sm" />
            </div>
            <div>
              <label className="block text-xs font-bold text-muted-col uppercase tracking-wider mb-2">Duration (seconds)</label>
              <input type="number" value={duration} onChange={(e) => setDuration(parseInt(e.target.value) || 30)} className="w-full px-4 py-3 rounded-xl input-glass text-sm" />
            </div>
          </div>

          <div>
            <label className="block text-xs font-bold text-muted-col uppercase tracking-wider mb-2">Tags</label>
            <input type="text" value={tags} onChange={(e) => setTags(e.target.value)} placeholder="greeting, daily-conversation, beginner" className="w-full px-4 py-3 rounded-xl input-glass text-sm" />
          </div>

          <div>
            <div className="flex items-center justify-between mb-3">
              <label className="text-xs font-bold text-muted-col uppercase tracking-wider">Script Sentences</label>
              <button onClick={addSegment} className="text-xs font-medium text-primary hover:underline">+ Add Sentence</button>
            </div>
            <div className="space-y-4">
              {segments.map((seg, i) => (
                <div key={seg.id} className="p-4 rounded-xl glass-surface space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-muted-col">Sentence {i + 1}</span>
                    {segments.length > 1 && (
                      <button onClick={() => removeSegment(i)} className="text-xs text-[var(--status-rejected)] hover:underline">Remove</button>
                    )}
                  </div>
                  <input type="text" value={seg.text} onChange={(e) => updateSegment(i, "text", e.target.value)} placeholder="Japanese sentence" className="w-full px-3 py-2 rounded-lg input-glass text-sm" />
                  <input type="text" value={seg.translation} onChange={(e) => updateSegment(i, "translation", e.target.value)} placeholder="English translation" className="w-full px-3 py-2 rounded-lg input-glass text-sm" />
                  <div className="grid grid-cols-2 gap-2">
                    <input type="number" value={seg.startTime} onChange={(e) => updateSegment(i, "startTime", parseInt(e.target.value) || 0)} placeholder="Start (s)" className="px-3 py-1.5 rounded-lg input-glass text-xs" />
                    <input type="number" value={seg.endTime} onChange={(e) => updateSegment(i, "endTime", parseInt(e.target.value) || 0)} placeholder="End (s)" className="px-3 py-1.5 rounded-lg input-glass text-xs" />
                  </div>
                </div>
              ))}
            </div>
