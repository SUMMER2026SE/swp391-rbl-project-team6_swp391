...
const inputBase =
  "w-full px-3 py-1.5 rounded-lg border text-sm outline-none focus:ring-2 focus:ring-indigo-400/40 transition-colors duration-200 bg-white dark:bg-slate-900/80 border-slate-200 dark:border-white/10 text-slate-900 dark:text-white placeholder:text-slate-400";

function ProfilePage() {
  const [activeTab, setActiveTab] = useState<"overview" | "achievements" | "settings">("overview");
  const [settingsSection, setSettingsSection] = useState<"account" | "appearance" | "security">(
    "account",
  );
  ...
  const avatarLetter =
    (profile?.displayName || user?.name || user?.email || "").trim().charAt(0).toUpperCase() || "?";
      ...
      <div className="flex items-center justify-center py-20">
        <div className="text-center space-y-3 max-w-sm">
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg bg-indigo-500 text-white text-sm font-bold hover:opacity-90 transition"
          >
            Retry
          </button>
        </div>
            ...
            {editing ? (
              <>
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-white/20 text-white/80 text-xs font-semibold backdrop-blur-sm hover:bg-white/30 transition disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
                  onClick={handleSave}
                  disabled={isProfileSaving}
                  className="px-2.5 py-1 rounded-lg bg-white text-purple-600 text-xs font-bold backdrop-blur-sm shadow hover:bg-white/90 transition disabled:opacity-60 flex items-center gap-1"
                >
                  {isProfileSaving ? (
                    <>
                ...
                <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 z-20">
                  <div className="relative">
                    <button
                      ...
                      className="w-6 h-6 rounded-full bg-white dark:bg-slate-800 shadow border border-slate-200 dark:border-white/10 flex items-center justify-center hover:bg-slate-50 dark:hover:bg-slate-700 transition disabled:opacity-50"
                    >
                      {isAvatarSaving ? (
              ...
              <div className="flex-1 w-full text-center sm:text-left">
                ...
                {editing && (
                  <div className="mt-1.5">
                    <label className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 mb-1 block">
                      Location
                    </label>
                    <input
                      value={editLocation}
          ...
          <div className="text-center p-2.5 rounded-xl bg-white/90 dark:bg-slate-800/50 border border-slate-200/70 dark:border-white/5 shadow-sm hover:shadow transition-colors duration-300">
            ...
            <div className="flex justify-center gap-0.5 mt-1.5">
              {[...Array(7)].map((_, i) => (
                <div
                  key={i}
                  className="w-1.5 h-1.5 rounded-full bg-slate-200/80 dark:bg-slate-700"
                />
              ))}
            </div>
          </div>
              ...
              <div className="space-y-2">
                ...
                <div>
                  ...
                  <p className="text-[9px] text-slate-400 dark:text-slate-500 mt-0.5">
                    Max 2–3 sentences about your Japanese learning goals.
                  </p>
                </div>
                <div>
                  <label className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 block mb-1">
              ...
              {editing && (
                <div className="flex gap-2 mt-3">
                  <button
                    ...
                    disabled={isProfileSaving}
                    className="flex-1 py-2 rounded-xl border border-slate-200/80 dark:border-white/10 text-xs font-semibold text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition disabled:opacity-50"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={handleSave}
                    disabled={isProfileSaving}
                    className="flex-1 py-2 rounded-xl bg-gradient-to-r from-indigo-500 to-pink-500 text-white text-xs font-bold shadow-md shadow-indigo-500/20 hover:opacity-90 transition disabled:opacity-60 flex items-center justify-center gap-1.5"
                  >
                    {isProfileSaving ? (
                      <>
                        <Loader2 className="w-3 h-3 animate-spin" /> Saving...
                      ...
                      <div className="mt-1 space-y-1">
                        {validationRules.map((rule) => (
                          <div key={rule.label} className="flex items-center gap-1.5">
                            <div
                              className={`w-3 h-3 rounded-full flex items-center justify-center flex-shrink-0 ${rule.met ? "bg-green-100 dark:bg-green-500/20" : "bg-slate-100 dark:bg-slate-800"}`}
                            >
                              {rule.met ? (
                            ...
                            <span
                              className={`text-[10px] font-medium ${rule.met ? "text-green-600 dark:text-green-400" : "text-slate-400 dark:text-slate-500"}`}
                            >
                              {rule.label}
                            </span>
                          </div>
                        ...
                        {getStrength(newPw) && (
                          <div className="flex items-center gap-2 mt-0.5">
                            <div className="flex-1 h-1 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                              <motion.div
                                animate={{ width: `${getStrength(newPw)!.pct}%` }}
                                className={`h-full rounded-full ${getStrength(newPw)!.color}`}
                              />
                            </div>
                            <span
                              className={`text-[10px] font-black ${getStrength(newPw)!.label === "Weak" ? "text-red-500" : getStrength(newPw)!.label === "Medium" ? "text-amber-500" : "text-green-500"}`}
                            >
