          ...
          <Card className="p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div
                  key={i}
                  className={`rounded-xl p-3 text-center ${stat.color} flex flex-col justify-center`}
                >
                  <div className="text-xl font-black">{stat.value}</div>
                  <div className="text-[10px] uppercase font-bold tracking-wider mt-1 opacity-80">
                    {stat.label}
                  </div>
                </div>
              ))}
            ...
            <Card className="p-5 h-full">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {[
                  {
                    name: "Vocabulary",
                    val: stats?.progressPercent ? Math.round(stats.progressPercent) : 75,
                    count: `${stats?.learnedWords ?? 0} learned`,
                  },
                  {
                    name: "Grammar",
                    val: stats?.grammarCompleted
                      ? Math.round((stats.grammarCompleted / 100) * 100)
                      : 60,
                    count: `${stats?.completedLessons ?? 0} completed`,
                  },
