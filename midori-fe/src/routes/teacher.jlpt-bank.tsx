    ...
    <div className="mx-auto max-w-7xl space-y-6">
        ...
        actions={
          <Button asChild>
            <Link to="/teacher/exams/create?source=jlpt-bank">
              <Sparkles className="mr-2 h-4 w-4" />Create from JLPT bank
            </Link>
          </Button>
        }
      />
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative min-w-0 flex-1 sm:max-w-xs">
          ...
          <Input placeholder="Search sets…" value={q} onChange={(e) => setQ(e.target.value)} className="pl-9" />
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {filtered.map((s) => (
          <Card key={s.id} className="border-border/60 transition-all hover:border-primary/40 hover:shadow-md">
            <CardContent className="p-5">
              <div className="mb-2 flex items-center gap-2">
                <LevelBadge level={s.level} />
                <span className="text-xs text-muted-foreground">{s.year}</span>
              </div>
              <button onClick={() => setOpen(s.id)} className="block text-left font-display text-lg font-semibold hover:text-primary">{s.title}</button>
              ...
              <div className="mt-3 grid grid-cols-4 gap-2 text-center text-[10px]">
                ...
                <div className="rounded-md bg-muted/40 p-2">
                  ...
                  <div className="text-sm font-bold">{s.mix.easy}/{s.mix.medium}/{s.mix.hard}</div>
                </div>
              </div>
              <div className="mt-4 flex gap-2">
                ...
                <Button asChild className="flex-1">
                  <Link to={`/teacher/exams/create?source=jlpt-bank&jlptSetId=${s.id}`}>
                    <FileBadge className="mr-2 h-4 w-4" />Use set
                  </Link>
                </Button>
              </div>
            </CardContent>
          ...
          <div className="space-y-3 text-sm">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                <span><DifficultyBadge d="Easy" /> {sel.mix.easy}%</span>
                <span><DifficultyBadge d="Medium" /> {sel.mix.medium}%</span>
                <span><DifficultyBadge d="Hard" /> {sel.mix.hard}%</span>
              </div>
            </div>
            <Button asChild className="w-full">
              <Link to={`/teacher/exams/create?source=jlpt-bank&jlptSetId=${sel.id}`}>Use this set</Link>
            </Button>
          </div>
        )}
      </PreviewSheet>
