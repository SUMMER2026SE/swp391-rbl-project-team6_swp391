  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <PageHeader
        ...
        actions={<Button asChild><Link to="/teacher/homework/create"><Plus className="mr-2 h-4 w-4" />Assign homework</Link></Button>}
      />

      <div className="flex flex-wrap items-center gap-2">
        <div className="relative min-w-0 flex-1 sm:max-w-xs">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input placeholder="Search homework..." value={q} onChange={(e) => setQ(e.target.value)} className="pl-9" />
        </div>
            ...
            <Card key={h.id}>
              <CardContent className="grid items-center gap-3 p-4 sm:grid-cols-[minmax(0,1fr)_180px_auto]">
                <div className="min-w-0">
                  <div className="mb-1 flex items-center gap-2"><LevelBadge level={cls.level} /><StatusBadge status={h.status} /><span className="text-[10px] uppercase tracking-widest text-muted-foreground">{h.source.replace("-", " ")}</span></div>
                  <Link to="/teacher/classes/$classId/homework" params={{ classId: cls.id }} className="block truncate font-semibold hover:text-primary">{h.title}</Link>
                  <div className="text-xs text-muted-foreground">{cls.name} · Due {h.dueDate}</div>
                </div>
                <div>
                ...
                <Button asChild variant="outline" size="sm"><Link to="/teacher/classes/$classId/homework" params={{ classId: cls.id }}>Open</Link></Button>
              </CardContent>
            </Card>
          );
        })}
