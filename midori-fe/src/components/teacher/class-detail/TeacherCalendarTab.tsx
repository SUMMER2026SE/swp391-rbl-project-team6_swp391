...
interface TeacherCalendarTabProps {
  classInfo: TeacherClassInfo;
}

export function TeacherCalendarTab({ classInfo }: TeacherCalendarTabProps) {
    ...
    <div className="space-y-4">
      <Card className="p-5">
        <h3 className="font-display font-bold text-base text-foreground dark:text-white mb-4 flex items-center gap-2">
          ...
          Classroom Deadlines & Events Calendar
        </h3>
        ...
        <div className="space-y-3">
          ...
          {events.length === 0 && (
            <p className="text-xs text-muted-foreground text-center py-8">
              No calendar events or deadlines scheduled.
            </p>
          )}
        </div>
      </Card>
