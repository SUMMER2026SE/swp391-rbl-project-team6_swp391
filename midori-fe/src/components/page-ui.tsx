...
export function StatCard({
  ...
  hint?: string;
  icon?: ReactNode;
  accent?: "primary" | "sakura" | "sky" | "red";
}) {
  const ring = {
    primary: "bg-primary/15 text-primary",
    sakura: "bg-sakura/40 text-jp-red",
    sky: "bg-sky-blue/20 text-sky-blue",
    red: "bg-jp-red/15 text-jp-red",
  }[accent];
  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      className="bg-card text-card-foreground border border-border/50 rounded-2xl p-5 shadow-sm"
    >
      <div className="flex items-center gap-3">
        {icon && (
          <div className={`w-10 h-10 rounded-xl grid place-items-center ${ring}`}>{icon}</div>
        )}
...
export function Card({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <div
      className={`rounded-2xl p-5 border border-border/50 shadow-sm bg-card text-card-foreground ${className}`}
    >
      {children}
    </div>
  );
...
export function LevelBadge({ level }: { level: string }) {
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${colors[level] || "bg-muted"}`}
    >
      {level}
    </span>
  );
