...
public class AdminAiContentServiceImpl implements AdminAiContentService {
    ...
    private AdminAiContentGenerateResponse generateReadingContent(
        ...
        try {
            ...
            int filteredQuestionsCount = 0;
            int filteredPassagesCount = 0;
                ...
                if (passage != null && passage.getContent() != null && !passage.getContent().isBlank()) {
                    if (passage.getQuestions() != null) {
                        ...
                        for (AdminReadingAiDraft.QuestionDraft q : passage.getQuestions()) {
                            ...
                            List<AdminReadingAiDraft.OptionDraft> opts = q.getOptions();
                            if (opts == null || opts.size() != 4) {
                                filteredQuestionsCount++;
                                continue;
                            }
                            long correctCount = opts.stream().filter(o -> o != null && Boolean.TRUE.equals(o.getIsCorrect())).count();
                            if (correctCount != 1) {
                                filteredQuestionsCount++;
                                continue;
                            }
                    ...
                    // A passage with content but null questions is a blank/incomplete content issue.
                    // A passage with content but [] (empty) questions is a "zero valid questions" issue.
                    if (passage.getQuestions() == null) {
                        filteredPassagesCount++;
                        filteredQuestionsCount += 0;
                        log.debug("Filtering reading passage with null questions array (AI forgot questions)");
                    } else if (passage.getQuestions().isEmpty()) {
                        filteredPassagesCount++;
                        filteredQuestionsCount += 0;
                        log.debug("Filtering reading passage with zero questions");
                    } else {
                        validPassages.add(passage);
                    }
                } else {
                    filteredPassagesCount++;
                    log.debug("Filtering reading passage with blank or null content");
                }
            }

            if (validPassages.isEmpty()) {
                throw new AiProcessingException(
                        String.format("AI returned %d reading passage(s); all were filtered out "
                                + "(%d invalid content, %d with no valid questions). "
                                + "Please provide more coherent source content.",
                                originalPassagesCount, filteredPassagesCount, filteredQuestionsCount));
            }

            draft.setPassages(validPassages);
            String warning = null;
            if (filteredPassagesCount > 0 || filteredQuestionsCount > 0) {
                warning = String.format(
                        "Filtered out %d incomplete passage(s) and %d invalid question(s). "
                                + "%d passage(s) and %d question(s) retained.",
                        filteredPassagesCount, filteredQuestionsCount,
                        validPassages.size(), validQuestionsCount);
            }
